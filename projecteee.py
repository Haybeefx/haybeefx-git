...
# # Create the transactions table if not already exists
# cursor.execute("""
...
#             time.sleep(1)
#             banking_menu(user[1])  # Pass the user_id to banking_menu
#         else:
...
#         result = cursor.execute("SELECT balance FROM users WHERE id = ?", (user_id,)).fetchone()
